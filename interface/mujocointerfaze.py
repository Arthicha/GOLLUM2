...
class MujocoInterfaze(MujocoEnv, utils.EzPickle):
	...
	#_home = np.array([[-0.2, 0.85, -1.56],[0.2, 0.85, -1.56],[-0.2, 0.85, -1.56],[0.2, 0.85, -1.56]])
	
	_home = np.array([[-0.2, 0.608813, -1.21763],[0.2, 0.608813, -1.21763],[-0.2, 0.608813, -1.21763],[0.2, 0.608813, -1.21763]])
	#_home = np.array([[-0.2, 0.78, -2.0],[0.2, 0.78, -2.0],[-0.2, 0.78, -2.0],[0.2, 0.78, -2.0]])
	...
	def set_robot_joint(self,target_pos):
		...
		self.__target_positions = target_pos.reshape((4,3))*self.mask + self._home
	...
	def get_observations(self):
		...
		velo = (pose - self.previous_pos)/self.model.opt.timestep
		orien = pose[3:]

		return np.concatenate([orien,velo],axis=0)
	...
	def get_jointangle(self):
		obs = np.concatenate((np.array(self.data.joint("FR_hip_joint").qpos),
							  np.array(self.data.joint("FR_thigh_joint").qpos),
							  np.array(self.data.joint("FR_calf_joint").qpos),
							  np.array(self.data.joint("FL_hip_joint").qpos),
							  np.array(self.data.joint("FL_thigh_joint").qpos),
							  ...
							  np.array(self.data.joint("RL_hip_joint").qpos),
							  np.array(self.data.joint("RL_thigh_joint").qpos),
							  np.array(self.data.joint("RL_calf_joint").qpos)), axis=0)
		return obs


	def get_jointtorque(self):
		tau = np.concatenate((self.get_eachtorque("FR_hip_joint"),
							  self.get_eachtorque("FR_thigh_joint"),
							  self.get_eachtorque("FR_calf_joint"),
							  self.get_eachtorque("FL_hip_joint"),
							  self.get_eachtorque("FL_thigh_joint"),
							  self.get_eachtorque("FL_calf_joint"),
							  self.get_eachtorque("RR_hip_joint"),
							  self.get_eachtorque("RR_thigh_joint"),
							  self.get_eachtorque("RR_calf_joint"),
							  self.get_eachtorque("RL_hip_joint"),
							  self.get_eachtorque("RL_thigh_joint"),
							  self.get_eachtorque("RL_calf_joint")), axis=0)
		return tau

	def get_eachtorque(self,jname):
		return np.array(self.data.joint(jname).qfrc_smooth) #+ np.array(self.data.joint(jname).qfrc_constraint)
	...
	def step(self,action,contact=np.array([1.0,1.0,1.0,1.0]),ki=0.0):
		# get observation
		obs = self.get_obs()
		roll_t = obs[0]
		pitch_t = obs[1]
		...
		#current_vel = (current_pos - self.previous_pos)/self.model.opt.timestep
		#current_acc = (current_vel - self.previous_vel)/self.model.opt.timestep
		...
		# ------------  target hip adaptation ----------------
		#target_t[:,0] = self.get_hip_adaptation(target_t[:,1],target_t[:,2])
		...
		if ki > 0.0:
			error = target_t - theta_t
			mask = (error * self.integrated_error < 0)
			self.integrated_error += error
			self.integrated_error[mask] *= 0

			tau0_t += ki * self.integrated_error 
	...
	def softreset(self,render=False):
		self.step_number = 0
		T0 = int(0.5/self.model.opt.timestep)


		targets = np.zeros((4,4,3))
		contacts = np.zeros((4,4))+1 
		for l in range(4):
			targets[l] = deepcopy(self._home)
			targets[l,[0,1,0,1][l],1:] = np.array([0.85, -1.85])
			contacts[l,[0,1,0,1][l]] = 0
			
			
			targets[l,[3,2,3,2][l],1:] = np.array([0.85, -1.85])
			contacts[l,[3,2,3,2][l]] = 0
		
		
		homecontacts = np.ones((4))
		timestamp = np.array([0] + np.arange(T0,4*T0+1,T0//4).tolist())
		targets_ = np.array([self._home]+[self._home,targets[0],self._home,self._home,targets[1],\
			self._home,self._home,targets[2],self._home,self._home,targets[3]]+[self._home,self._home])

		contacts_ = np.array([homecontacts]+[homecontacts,contacts[0],homecontacts,homecontacts,contacts[1],\
			homecontacts,homecontacts,contacts[2],homecontacts,homecontacts,contacts[3]]+[homecontacts,homecontacts])
		
		
		jointtarget = np.zeros((4*T0,4,3))
		footcontact = np.zeros((4*T0,4))
		for l in range(4):
			for j in range(3):
				jointtarget[:,l,j] = np.interp(np.arange(0,4*T0,1),timestamp,targets_[:,l,j])
			footcontact[:,l] = np.interp(np.arange(0,4*T0,1),timestamp,contacts_[:,l])
		footcontact[footcontact < 1] *= 0
		#print(footcontact)

		for t in range(4*T0):
			#print(footcontact[t])
			self.step(jointtarget[t],contact=footcontact[t])
			if render:
				if t % (self.hz//5) == 0:
					cv2.imshow("image", self.render())
					cv2.waitKey(1)
		...
		for t in range(2000):
			self.step(self._home,contact=homecontacts,ki=0.5)
			if render:
				if t % (self.hz//5) == 0:
					cv2.imshow("image", self.render())
					cv2.waitKey(1)
		...
		'''
		plt.subplot(2,1,1)
		plt.plot(jointtarget[:,0,:])
		plt.subplot(2,1,2)
		plt.plot(footcontact)
	...
	def estimate_legphase(self,target,theta,roll,pitch,contact=np.array([1,1,1,1])):
		...
		# combined moment,error-based
		#weighting_gains =  phase*(rs/np.sum(rs)) #* np.power(phase,2)
		#weighting_gains =  (1-np.clip(rs/np.max(rs),0,1)) *phase # np.power(phase,2)
		#weighting_gains /= EPS + np.sum(weighting_gains)

		#return weighting_gains
	...
	def get_effective_length(self,q2, q3):
		return self.upperlength * np.cos(q2) + self.lowerlength * np.cos(q2 + q3)

	def get_hip_adaptation(self,q2, q3):
		gain = self.get_effective_length(self._home[:,1], self._home[:,2]) / self.get_effective_length(q2, q3)
		q1 = np.arcsin(np.clip(gain * np.sin(self._home[:,0]),-1,1))
		return np.clip(q1, -0.4, 0.4)
