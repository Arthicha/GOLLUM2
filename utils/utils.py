...
class SoftTorchReplay:
	...
	def softadd(self,idx,value,convert=False):
		value_ = torch.FloatTensor(value).to(self.device) if convert else value
		self.__sid = idx
		self.__subdata[self.__sid] = value_.clone()
...
class RunningMeanStd(object):
	...
	def update_from_moments(self, batch_mean, batch_var, batch_count):
		...
		#self.count = np.clip(self.count,0,2048*100)
...
def make_transition(state,action,reward,next_state,done,enable=1,activity1=None,activity2=None):
	transition = {}
...
class ReplayBuffer():
	def __init__(self, action_prob_exist, max_size, state_dim, num_action,reward_dim=3):
	...
	def sample(self, shuffle, batch_size = None):
		if shuffle :
			...
			rand_idx = np.random.choice(sample_num, batch_size,replace=True)
			sampled_data = {}
